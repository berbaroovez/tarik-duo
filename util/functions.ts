...
const checkForEmotes = async (textArray: CustomSpan[]) => {
  ...
  console.log("twitchChannelEmoteData", twitchChannelEmoteData)
  ...
  //this loop checks emotes for twitch  channel emotes
    ...
    for (const word of textArray) {
      if (emote.name == word.text) {
        ...
        if (emote.format.includes("animated")) {
          let tempUrl = emote.images["url_1x"].split("/static/")
          word.url = tempUrl[0] + "/default/light/1.0"
        } else {
          word.url = emote.images["url_1x"]
        }
      }
...
const getFossabotCommand = async (
  commandName: string
): Promise<string | null> => {
...
const addTagToPage = async (label: string) => {
  ...
  if (commandResponse !== null) {
    ...
    if (showDuoInSidebar) {
      if (label === "duo") {
        // console.log("adding duo to sidebar")
